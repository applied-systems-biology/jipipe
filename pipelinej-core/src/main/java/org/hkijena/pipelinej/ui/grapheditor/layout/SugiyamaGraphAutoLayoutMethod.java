...
package org.hkijena.pipelinej.ui.grapheditor.layout;
...
public class SugiyamaGraphAutoLayoutMethod implements GraphAutoLayoutMethod {
    ...
    public void accept(ACAQGraphCanvasUI canvasUI) {
        ...
        for (ACAQGraphEdge edge : graph.getGraph().edgeSet()) {
            ACAQNodeUI sourceUI = canvasUI.getNodeUIs().getOrDefault(graph.getGraph().getEdgeSource(edge).getNode(), null);
            ACAQNodeUI targetUI = canvasUI.getNodeUIs().getOrDefault(graph.getGraph().getEdgeTarget(edge).getNode(), null);
