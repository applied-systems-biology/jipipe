...
public class JIPipeDesktopGraphNodeSlotEditorUI extends JPanel implements JIPipeGraphNode.NodeSlotsChangedEventListener, JIPipeParameterCollection.ParameterChangedEventListener {
    ...
    private void initializeToolbar() {
        ...
        if (canModifyInputSlots()) {
            ...
            addInputButton.addActionListener(e -> {
                if (!JIPipeDesktopProjectWorkbench.canModifySlots(editorUI.getDesktopWorkbench()))
                    return;
                JIPipeDesktopAddAlgorithmSlotPanel.showDialog(this, editorUI.getCanvasUI().getHistoryJournal(), node, JIPipeSlotType.Input);
            });
        ...
        if (canModifyOutputSlots()) {
            ...
            addOutputButton.addActionListener(e -> {
                if (!JIPipeDesktopProjectWorkbench.canModifySlots(editorUI.getDesktopWorkbench()))
                    return;
                JIPipeDesktopAddAlgorithmSlotPanel.showDialog(this, editorUI.getCanvasUI().getHistoryJournal(), node, JIPipeSlotType.Output);
            });
    ...
    private void editSlot() {
        ...
        if (slot.getSlotType() == JIPipeSlotType.Input && canModifyInputSlots()) {
            JIPipeDesktopEditAlgorithmSlotPanel.showDialog(this, editorUI.getCanvasUI().getHistoryJournal(), slot);
