...
//            getParent().revalidate();
    ...
    public JIPipeDesktopGraphEdgeUI pickEdgeUI(MouseEvent mouseEvent) {
        ...
        return edgeUIs.values().stream()
                .filter(edgeUI -> isMouseOverEdge(mouseX, mouseY, edgeUI))
                .findFirst()
                .orElse(null);
    }
     ...
     */
    private boolean isMouseOverEdge(int mouseX, int mouseY, JIPipeDesktopGraphEdgeUI edgeUI) {

        int hitThreshold = (int) Math.max(1, zoom * 4) + 4;
...
//        System.out.println(lineRectangle);
    ...
    private boolean isMouseOverElbowEdge(int mouseX, int mouseY, Point sourcePoint, Rectangle targetBounds, Point targetPoint, int hitThreshold) {
        ...
        getPaintManager().createElbowEdgeCoordinates(sourcePoint, targetBounds, targetPoint, 1, 0, 0, true, xCoords, yCoords);
