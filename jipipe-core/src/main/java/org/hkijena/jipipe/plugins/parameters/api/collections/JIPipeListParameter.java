...
public abstract class JIPipeListParameter<T> extends ArrayList<T> implements JIPipeValidatable, JIPipeCustomTextDescriptionParameter {
     ...
     */
    public JIPipeListParameter(Class<T> contentClass) {
        this.contentClass = contentClass;
    }

    public JIPipeListParameter(JIPipeListParameter<T> other) {
     ...
     */
    public static class Serializer extends JsonSerializer<JIPipeListParameter<?>> {
        @Override
        public void serialize(JIPipeListParameter<?> objects, JsonGenerator jsonGenerator, SerializerProvider serializerProvider) throws IOException, JsonProcessingException {
     ...
     */
    public static class Deserializer<T> extends JsonDeserializer<JIPipeListParameter<T>> implements ContextualDeserializer {

        private JavaType deserializedType;

        @Override
        public JIPipeListParameter<T> deserialize(JsonParser jsonParser, DeserializationContext deserializationContext) throws IOException, JsonProcessingException {
            JsonNode root = jsonParser.readValueAsTree();

            JIPipeListParameter<T> listParameter;
            try {
                listParameter = (JIPipeListParameter<T>) deserializedType.getRawClass().newInstance();
            } catch (InstantiationException | IllegalAccessException e) {
                throw new RuntimeException(e);
            }
