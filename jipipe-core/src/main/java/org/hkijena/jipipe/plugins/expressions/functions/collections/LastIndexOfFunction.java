...
import static org.hkijena.jipipe.plugins.expressions.functions.EvaluateFunction.parseVariableAssignment;

@SetJIPipeDocumentation(name = "Last index of", description = "Finds the last index of the array where the condition applies. Returns -1 if none match.")
public class LastIndexOfFunction extends ExpressionFunction {
    ...
    public Object evaluate(List<Object> parameters, JIPipeExpressionVariablesMap variables) {
        ...
        for (int i = 0; i < sequence.size(); ++i) {
            ...
            boolean test = JIPipeExpressionParameter.getEvaluatorInstance().test(loopedExpression, localVariables);
            if (test) {
                result = i;
            }
        }
