...
@JIPipeDataStorageDocumentation(
        @Entity(id = "glob://./*.py", type = EntityType.File, name = "Python script", description = "The Python script file", encodingFormat = EncodingFormats.PYTHON)
)
public class PythonScriptData extends StringData {
    ...
    public PythonScriptData(StringData other) {
        super(other);
    }

    public static PythonScriptData importData(JIPipeReadDataStorage storage, JIPipeProgressInfo progressInfo) {
        Path file = PathUtils.findFileByExtensionIn(storage.getFileSystemPath(), ".py");
        try {
            return new PythonScriptData(new String(Files.readAllBytes(file), Charsets.UTF_8));
        } catch (IOException e) {
