 ...
 */
public class JIPipeNodeTemplateSimpleListCellRenderer extends JPanel implements ListCellRenderer<JIPipeNodeTemplate> {
     ...
     */
    public JIPipeNodeTemplateSimpleListCellRenderer() {
        setOpaque(true);
    ...
    @Override
    public Component getListCellRendererComponent(JList<? extends JIPipeNodeTemplate> list, JIPipeNodeTemplate info, int index, boolean isSelected, boolean cellHasFocus) {

        setFont(list.getFont());

        if (info != null) {
            nodeColor.setFillColor(ThemeUtils.getNodeFillColor(info.getColorHue()));
            descriptionLabel.setText(info.getDescription().getHtml());
            nameLabel.setText(info.getName());
            nodeIcon.setIcon(JIPipe.RESOURCES.getIcon16(info.getIcon().getIconName()));
            sourceLabel.setText(StringUtils.nullToEmpty(info.getSource()));
