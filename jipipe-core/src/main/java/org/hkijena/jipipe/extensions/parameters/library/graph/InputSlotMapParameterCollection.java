...
public class InputSlotMapParameterCollection extends SlotMapParameterCollection {
     ...
     * @param newInstanceGenerator optional method that generated new instances. Can be null
     */
    public InputSlotMapParameterCollection(Class<?> dataClass, JIPipeGraphNode algorithm, Function<JIPipeDataSlotInfo, Object> newInstanceGenerator) {
     ...
     * @param algorithm the algorithm that contains the slots
     */
    public InputSlotMapParameterCollection(Class<?> dataClass, JIPipeGraphNode algorithm) {
    ...
    public void updateSlots() {
        if (getNode() != null) {
            ...
            for (String slotName : getParameters().keySet()) {
                if (!getNode().hasInputSlot(slotName)) {
            ...
            for (int i = 0; i < getNode().getInputSlots().size(); i++) {
                JIPipeDataSlot slot = getNode().getInputSlots().get(i);
