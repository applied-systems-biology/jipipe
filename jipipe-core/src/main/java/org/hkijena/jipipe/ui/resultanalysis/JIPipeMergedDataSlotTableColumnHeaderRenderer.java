...
public class JIPipeMergedDataSlotTableColumnHeaderRenderer implements TableCellRenderer {
    ...
    public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus, int row, int column) {
        ...
        if (column < 5) {
        ...
        } else if(column < dataTable.getDataAnnotationColumns().size() + 5) {
            String info = dataTable.getDataAnnotationColumns().get(column - 5);
            ...
            String info = dataTable.getAnnotationColumns().get(column - dataTable.getDataAnnotationColumns().size() - 5);
            String html = String.format("<html><table><tr><td><img src=\"%s\"/></td><td>%s</tr>",
