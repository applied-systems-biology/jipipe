...
public class JIPipeCacheBrowserUI extends JIPipeProjectWorkbenchPanel {
    ...
    private void showDataSlotsOfState(JIPipeGraphNode algorithm, JIPipeProjectCacheState state) {
        ...
        Map<JIPipeProjectCacheState, Map<String, JIPipeDataSlot>> stateMap = getProject().getCache().extract((JIPipeAlgorithm) algorithm);
        if (stateMap != null) {
    ...
    private void showDataSlots(List<JIPipeDataSlot> slots) {
        JIPipeCacheMultiDataSlotTableUI ui = new JIPipeCacheMultiDataSlotTableUI(getProjectWorkbench(), slots, true);
