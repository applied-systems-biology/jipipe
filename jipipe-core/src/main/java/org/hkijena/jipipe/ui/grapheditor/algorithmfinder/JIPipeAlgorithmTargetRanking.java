 ...
 */
public class JIPipeAlgorithmTargetRanking implements RankingFunction<Object> {
    ...
    public int[] rank(Object value, String[] filterStrings) {
        ...
        if (value instanceof JIPipeGraphNode) {
            ...
            for (JIPipeDataSlot targetSlot : node.getInputSlots()) {
                ...
                if (!sourceSlot.getNode().getParentGraph().getInputIncomingSourceSlots(targetSlot).isEmpty()) {
                    ranks[3] = 1;
                }
            }
        } else {
