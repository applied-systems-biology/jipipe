 ...
 */
@SetJIPipeDocumentation(name = "Custom auto threshold 2D (min+max, 32-bit)", description = "Allows to implement a custom thresholding method via expressions. " +
...
public class CustomAutoThreshold2D32Fv2Algorithm extends JIPipeIteratingAlgorithm {
     ...
     * @param info the info
     */
    public CustomAutoThreshold2D32Fv2Algorithm(JIPipeNodeInfo info) {
        super(info);
        ...
        maxThresholdParameters.getThresholdCalculationExpression().setExpression("NUM(\"inf\")");
     ...
     */
    public CustomAutoThreshold2D32Fv2Algorithm(CustomAutoThreshold2D32Fv2Algorithm other) {
        super(other);
    ...
    private void thresholdCombineThresholds(JIPipeSingleIterationStep iterationStep, JIPipeProgressInfo progressInfo, ImagePlus inputImage, ImagePlus outputImage, JIPipeExpressionVariablesMap parameters, ROIListData finalRoiInput, ImagePlus finalMaskInput) {
        ...
        ImageJUtils.forEachIndexedZCTSlice(inputImage, (ip, index) -> {
            ...
            float minThreshold = minThresholdParameters.getThreshold(parameters, statistics, accessPixels, pixels);
            float maxThreshold = maxThresholdParameters.getThreshold(parameters, statistics, accessPixels, pixels);
...

        {
            ...
            minThreshold = combined.floatValue();
        ...
        }
        {
            ...
            maxThreshold = combined.floatValue();
    ...
    private void applyThreshold(FloatProcessor source, ByteProcessor target, float minThreshold, float maxThreshold) {
    ...
    public static class ThresholdParameters extends AbstractJIPipeParameterCollection {
        ...
        public float getThreshold(JIPipeExpressionVariablesMap parameters, ImageStatistics statistics, boolean accessPixels, TFloatArrayList pixels) {
