...
import org.hkijena.jipipe.plugins.imageviewer.legacy.plugins3d.CalibrationPlugin3D;
import org.hkijena.jipipe.plugins.imageviewer.legacy.plugins3d.LUTManagerPlugin3D;
import org.hkijena.jipipe.plugins.imageviewer.legacy.plugins3d.OpacityManagerPlugin3D;
...
public class JIPipeDesktopLegacyImageViewerPanel3D extends JPanel implements JIPipeDesktopWorkbenchAccess, Disposable, UniverseListener, ComponentListener, JIPipeRunnable.FinishedEventListener {
    private final JIPipeDesktopLegacyImageViewer imageViewer;
    private final LegacyImageViewer3DUIApplicationSettings settings;
    ...
    public JIPipeDesktopLegacyImageViewerPanel3D(JIPipeDesktopLegacyImageViewer imageViewer) {
        this.imageViewer = imageViewer;
        if (JIPipe.getInstance() != null) {
            settings = LegacyImageViewer3DUIApplicationSettings.getInstance();
        } else {
            settings = new LegacyImageViewer3DUIApplicationSettings();
        }
    ...
    public LegacyImageViewer3DUIApplicationSettings getSettings() {
        return settings;
    }
...
//
    ...
    public void setImage(ImagePlusData image) {
        ...
        rebuildImageNow();
        for (JIPipeDesktopLegacyImageViewerPlugin3D plugin3D : getImageViewer().getPlugins3D()) {
    ...
    public void refreshFormPanel() {
        ...
        for (JIPipeDesktopLegacyImageViewerPlugin3D plugin : imageViewer.getPlugins3D()) {
...
//        canvas3D.flush();
    ...
    public static class UpdateLutAndCalibrationRun extends AbstractJIPipeRunnable {
        ...
        public UpdateLutAndCalibrationRun(JIPipeDesktopLegacyImageViewer imageViewer, List<Content> currentImageContents) {
            this.imageViewer = imageViewer;
...
//                }
    ...
    public static class UniverseInitializerRun extends AbstractJIPipeRunnable {
        ...
        private final JIPipeDesktopLegacyImageViewerPanel3D panel3D;
        private RendererStatus status = RendererStatus.ErrorGeneric;

        private CustomImage3DUniverse universe;

        public UniverseInitializerRun(JIPipeDesktopLegacyImageViewerPanel3D panel3D) {
    ...
    public static class ImageLoaderRun extends AbstractJIPipeRunnable {
        ...
        private final JIPipeDesktopLegacyImageViewerPanel3D viewerPanel3D;
        ...
        public ImageLoaderRun(JIPipeDesktopLegacyImageViewerPanel3D viewerPanel3D, ImagePlusData imageData, Image3DRenderType renderType, int resamplingFactor) {
        ...
        public void run() {
            ...
            for (JIPipeDesktopLegacyImageViewerPlugin3D plugin3D : viewerPanel3D.getImageViewer().getPlugins3D()) {
