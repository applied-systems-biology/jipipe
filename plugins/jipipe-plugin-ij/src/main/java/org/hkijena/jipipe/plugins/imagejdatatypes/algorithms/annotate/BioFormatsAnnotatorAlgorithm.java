...
package org.hkijena.jipipe.plugins.imagejdatatypes.algorithms.annotate;
 ...
 */
@SetJIPipeDocumentation(name = "Annotate file with OME metadata", description = "Annotates each file with information obtained from Bio-Formats without importing the file itself")
@AddJIPipeInputSlot(value = FileData.class, name = "Input", description = "The image file(s)", create = true)
@AddJIPipeOutputSlot(value = FileData.class, name = "Output", description = "The files, annotated with information obtained from Bio-Formats", create = true)
...
public class BioFormatsAnnotatorAlgorithm extends JIPipeSimpleIteratingAlgorithm {
    ...
    private OptionalDataAnnotationNameParameter omeXMLDataAnnotation = new OptionalDataAnnotationNameParameter("OMEXML", true);

    public BioFormatsAnnotatorAlgorithm(JIPipeNodeInfo info) {
        super(info);
        this.bioFormatsSettings = new BioFormatsSettings();
        registerSubParameter(bioFormatsSettings);
    }

    public BioFormatsAnnotatorAlgorithm(BioFormatsAnnotatorAlgorithm other) {
        super(other);
        this.bioFormatsSettings = new BioFormatsSettings(other.bioFormatsSettings);
        registerSubParameter(bioFormatsSettings);

        this.seriesIdsAnnotation = new OptionalTextAnnotationNameParameter(other.seriesIdsAnnotation);
        this.maxSeriesIdAnnotation = new OptionalTextAnnotationNameParameter(other.maxSeriesIdAnnotation);
        this.seriesNamesAnnotation = new OptionalTextAnnotationNameParameter(other.seriesNamesAnnotation);
        this.omeXMLDataAnnotation = new OptionalDataAnnotationNameParameter(other.omeXMLDataAnnotation);
    }
    ...
    protected void runIteration(JIPipeSingleIterationStep iterationStep, JIPipeIterationContext iterationContext, JIPipeGraphNodeRunContext runContext, JIPipeProgressInfo progressInfo) {
        ...
        try (IJLogToJIPipeProgressInfoPump pump = new IJLogToJIPipeProgressInfoPump(progressInfo)) {
            ...
            // Output file with all annotations
            ...
            seriesIdsAnnotation.addAnnotationIfEnabled(textAnnotations, JsonUtils.toJsonString(IntStream.range(0, process.getSeriesCount()).boxed().collect(Collectors.toList())));
            seriesNamesAnnotation.addAnnotationIfEnabled(textAnnotations, JsonUtils.toJsonString(IntStream.range(0, process.getSeriesCount()).boxed().map(i -> StringUtils.nullToEmpty(process.getSeriesLabel(i))).collect(Collectors.toList())));

            iterationStep.addOutputData(getFirstOutputSlot(), inputFile, textAnnotations, JIPipeTextAnnotationMergeMode.Merge, dataAnnotations, JIPipeDataAnnotationMergeMode.Merge, progressInfo);
    ...
    @SetJIPipeDocumentation(name = "Annotate with OME XML", description = "If enabled, create a data annotation with the OME XML data. Can be extracted with")
    @JIPipeParameter("ome-xml-data-annotation")
    public OptionalDataAnnotationNameParameter getOmeXMLDataAnnotation() {
        return omeXMLDataAnnotation;
    }

    @JIPipeParameter("ome-xml-data-annotation")
    public void setOmeXMLDataAnnotation(OptionalDataAnnotationNameParameter omeXMLDataAnnotation) {
        this.omeXMLDataAnnotation = omeXMLDataAnnotation;
    }

    @SetJIPipeDocumentation(name = "Annotate with max series ID", description = "If enabled, annotate with the maximum series ID")
    @JIPipeParameter("max-series-id-annotation")
    public OptionalTextAnnotationNameParameter getMaxSeriesIdAnnotation() {
        return maxSeriesIdAnnotation;
    }

    @JIPipeParameter("max-series-id-annotation")
    public void setMaxSeriesIdAnnotation(OptionalTextAnnotationNameParameter maxSeriesIdAnnotation) {
        this.maxSeriesIdAnnotation = maxSeriesIdAnnotation;
    }

    @SetJIPipeDocumentation(name = "Annotate with series IDs", description = "If enabled, annotate with the list of all series IDs")
    @JIPipeParameter("series-ids-annotation")
    public OptionalTextAnnotationNameParameter getSeriesIdsAnnotation() {
        return seriesIdsAnnotation;
    }

    @JIPipeParameter("series-ids-annotation")
    public void setSeriesIdsAnnotation(OptionalTextAnnotationNameParameter seriesIdsAnnotation) {
        this.seriesIdsAnnotation = seriesIdsAnnotation;
    }

    @SetJIPipeDocumentation(name = "Annotate with series labels", description = "If enabled, annotate with the list of series labels")
    @JIPipeParameter("series-names-annotation")
    public OptionalTextAnnotationNameParameter getSeriesNamesAnnotation() {
        return seriesNamesAnnotation;
    }

    @JIPipeParameter("series-names-annotation")
    public void setSeriesNamesAnnotation(OptionalTextAnnotationNameParameter seriesNamesAnnotation) {
        this.seriesNamesAnnotation = seriesNamesAnnotation;
    }
