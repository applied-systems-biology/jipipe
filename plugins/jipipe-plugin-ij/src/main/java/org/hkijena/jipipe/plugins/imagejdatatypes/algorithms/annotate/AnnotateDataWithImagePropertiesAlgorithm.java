 ...
 */
@SetJIPipeDocumentation(name = "Annotate data with image properties", description = "Adds data annotations generated from an image to any data.")
...
public class AnnotateDataWithImagePropertiesAlgorithm extends JIPipeIteratingAlgorithm {
     ...
     */
    public AnnotateDataWithImagePropertiesAlgorithm(JIPipeNodeInfo info) {
        super(info);
     ...
     */
    public AnnotateDataWithImagePropertiesAlgorithm(AnnotateDataWithImagePropertiesAlgorithm other) {
        super(other);
    ...
    protected void runIteration(JIPipeSingleIterationStep iterationStep, JIPipeIterationContext iterationContext, JIPipeGraphNodeRunContext runContext, JIPipeProgressInfo progressInfo) {
        ...
        if (getTitleAnnotation().isEnabled()) {
            annotations.add(new JIPipeTextAnnotation(getTitleAnnotation().getContent(), "" + image.getImage().getTitle()));
        }
        if (getWidthAnnotation().isEnabled()) {
            annotations.add(new JIPipeTextAnnotation(getWidthAnnotation().getContent(), "" + image.getImage().getWidth()));
        }
        if (getHeightAnnotation().isEnabled()) {
            annotations.add(new JIPipeTextAnnotation(getHeightAnnotation().getContent(), "" + image.getImage().getHeight()));
        }
        if (getStackSizeAnnotation().isEnabled()) {
            annotations.add(new JIPipeTextAnnotation(getStackSizeAnnotation().getContent(), "" + image.getImage().getNSlices()));
        }
        if (getPlaneNumberAnnotation().isEnabled()) {
            annotations.add(new JIPipeTextAnnotation(getPlaneNumberAnnotation().getContent(), "" + image.getImage().getStackSize()));
        }
        if (getChannelSizeAnnotation().isEnabled()) {
            annotations.add(new JIPipeTextAnnotation(getChannelSizeAnnotation().getContent(), "" + image.getImage().getNChannels()));
        }
        if (getFramesSizeAnnotation().isEnabled()) {
            annotations.add(new JIPipeTextAnnotation(getFramesSizeAnnotation().getContent(), "" + image.getImage().getNFrames()));
        }
        if (physicalDimensionXAnnotation.isEnabled()) {
            Calibration calibration = image.getImage().getCalibration();
            double value = 0;
            String unit = "";
            if (calibration != null) {
        ...
        if (getBitDepthAnnotation().isEnabled()) {
            annotations.add(new JIPipeTextAnnotation(getBitDepthAnnotation().getContent(), "" + image.getImage().getBitDepth()));
        }
        if (getColorSpaceAnnotation().isEnabled()) {
            String colorSpace = image.getColorSpace().toString();
