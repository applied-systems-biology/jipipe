 ...
 */
@SetJIPipeDocumentation(name = "TurboReg registration 2D (simple)", description = "Aligns slices in the input image to other slices using methods " +
...
@AddJIPipeOutputSlot(value = ImagePlusData.class, name = "Output", description = "The registered image", create = true)
@AddJIPipeOutputSlot(value = JsonData.class, name = "Transform", description = "The transform serialized in JSON format", create = true)
public class TurboRegRegistration2DSingleAlgorithm extends JIPipeIteratingAlgorithm {
    ...
    public TurboRegRegistration2DSingleAlgorithm(JIPipeNodeInfo info) {
        super(info);
        this.advancedTurboRegParameters = new AdvancedTurboRegParameters();
        registerSubParameter(advancedTurboRegParameters);

        // Add default config
        TurboRegRegistrationAlgorithmRule rule = new TurboRegRegistrationAlgorithmRule();
        rule.setReferenceCIndex(new JIPipeExpressionParameter("0"));
        rule.setReferenceZIndex(new JIPipeExpressionParameter("0"));
        rule.setReferenceTIndex(new JIPipeExpressionParameter("0"));
        rules.addFromTemplate(rule);
    }
    ...
    protected void runIteration(JIPipeSingleIterationStep iterationStep, JIPipeIterationContext iterationContext, JIPipeGraphNodeRunContext runContext, JIPipeProgressInfo progressInfo) {
        ImagePlus imp = iterationStep.getInputData("Input", ImagePlusData.class, progressInfo).getDuplicateImage();
        ...
        if (transformationType == TurboRegTransformationType.GenericTransformation) {
            ...
            iterationStep.addOutputData("Output",
                    new ImagePlusData(imp),
                    textAnnotations,
        ...
        for (int c = 0; c < imp.getNChannels(); c++) {
            for (int z = 0; z < imp.getNSlices(); z++) {
                for (int t = 0; t < imp.getNFrames(); t++) {
                    ...
                    AlignedImage5DSliceIndexExpressionParameterVariablesInfo.apply(variablesMap, imp, imp, sliceIndex);
        ...
        for (TurboRegRegistrationAlgorithmTransformationNode node : graph.vertexSet()) {
            if (node.getRule().getRuleType() == TurboRegRegistrationAlgorithmRuleType.UseTransformation) {
                AlignedImage5DSliceIndexExpressionParameterVariablesInfo.apply(variablesMap, imp, imp, node.getSourceIndex());
        ...
        while (topologicalOrderIterator.hasNext()) {
            ...
            ImageProcessor sourceIp = ImageJUtils.getSliceZero(imp, node.getSourceIndex());
            switch (node.getRule().getRuleType()) {
                ...
                case CalculateTransformation: {
                    ...
                    ImageSliceIndex targetIndex = ImageJUtils.toSafeZeroIndex(imp, new ImageSliceIndex(targetReferenceC, targetReferenceZ, targetReferenceT));
                    progressInfo.log("Aligning input slice " + node.getSourceIndex() + " to reference " + targetIndex);

                    ImageProcessor targetIp = ImageJUtils.getSliceZero(imp, targetIndex);
