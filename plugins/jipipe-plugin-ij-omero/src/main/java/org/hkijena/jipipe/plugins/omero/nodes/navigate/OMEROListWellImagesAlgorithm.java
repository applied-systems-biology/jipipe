...
public class OMEROListWellImagesAlgorithm extends JIPipeSingleIterationAlgorithm implements OMEROCredentialAccessNode {

    private OptionalOMEROCredentialsEnvironment overrideCredentials = new OptionalOMEROCredentialsEnvironment();
    private JIPipeExpressionParameter filters = new JIPipeExpressionParameter("");

    public OMEROListWellImagesAlgorithm(JIPipeNodeInfo info) {
        super(info);
    }

    public OMEROListWellImagesAlgorithm(OMEROListWellImagesAlgorithm other) {
        super(other);
    ...
    protected void runIteration(JIPipeMultiIterationStep iterationStep, JIPipeIterationContext iterationContext, JIPipeGraphNodeRunContext runContext, JIPipeProgressInfo progressInfo) {
        ...
        try (OMEROGateway gateway = new OMEROGateway(credentials, progressInfo)) {
            for (int row = 0; row < getFirstInputSlot().getRowCount(); row++) {
                ...
                progressInfo.log("Listing images in well ID=" + wellData.getId());

                for (WellSampleData wellSample : wellData.getWellSamples()) {
                    ImageData imageData = wellSample.getImage();
