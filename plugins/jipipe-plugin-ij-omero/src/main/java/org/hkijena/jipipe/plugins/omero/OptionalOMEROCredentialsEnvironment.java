...
public class OptionalOMEROCredentialsEnvironment extends OptionalParameter<OMEROCredentialsEnvironment> {
    ...
    public OMEROCredentialsEnvironment getContentOrDefault(OMEROCredentialsEnvironment defaultValue) {
        if (getContent() != null && getContent().generateValidityReport(new UnspecifiedValidationReportContext(), JIPipeValidationReportSettings.DEFAULT, JIPipeProgressInfo.SILENT).isValid()) {
            return getContent();
        } else {
            return defaultValue;
        }
