...
public class OMEROListWellsAlgorithm extends JIPipeSingleIterationAlgorithm {
                ...
                for (WellData wellData : gateway.getBrowseFacility().getWells(context, plateId)) {
                    if (wellData != null) {
                        ...
                        if (filters.test(variables)) {
                            getFirstOutputSlot().addData(new OMEROWellReferenceData(wellData, environment),
    ...
    @SetJIPipeDocumentation(name = "Keep well if", description = "Allows to filter the returned images")
    @JIPipeParameter("filter")
    @JIPipeExpressionParameterSettings(hint = "per OMERO well")
