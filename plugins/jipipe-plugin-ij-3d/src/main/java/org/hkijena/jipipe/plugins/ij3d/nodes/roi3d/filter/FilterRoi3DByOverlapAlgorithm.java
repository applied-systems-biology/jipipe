...
public class FilterRoi3DByOverlapAlgorithm extends JIPipeIteratingAlgorithm {
    ...
    private void updateSlots() {
        if (roi1Settings.isEnabled()) {
            if (!getOutputSlotMap().containsKey("ROI 1")) {
                ...
                slotConfiguration.addOutputSlot("ROI 1", "The first set of ROI", IJ3DROIListData.class, false);
            }
        } else {
            if (getOutputSlotMap().containsKey("ROI 1")) {
                JIPipeDefaultMutableSlotConfiguration slotConfiguration = (JIPipeDefaultMutableSlotConfiguration) getSlotConfiguration();
        ...
        if (roi2Settings.isEnabled()) {
            if (!getOutputSlotMap().containsKey("ROI 2")) {
                JIPipeDefaultMutableSlotConfiguration slotConfiguration = (JIPipeDefaultMutableSlotConfiguration) getSlotConfiguration();
                slotConfiguration.addOutputSlot("ROI 2", "The second set of ROI", IJ3DROIListData.class, false);
            }
        } else {
            if (getOutputSlotMap().containsKey("ROI 2")) {
                JIPipeDefaultMutableSlotConfiguration slotConfiguration = (JIPipeDefaultMutableSlotConfiguration) getSlotConfiguration();
    ...
    protected void runIteration(JIPipeSingleIterationStep iterationStep, JIPipeIterationContext iterationContext, JIPipeGraphNodeRunContext runContext, JIPipeProgressInfo progressInfo) {
        ...
        if (roi1Settings.isEnabled()) {
            ...
            IJ3DROIListData copy1 = new IJ3DROIListData();
            IJ3DROIListData copy2 = new IJ3DROIListData();
            copy1.addAll(roi1List);
            copy2.addAll(roi2List);
            IJ3DROIListData filtered = applyFilter(copy1, copy2, roi1Settings, imageHandler, variables, progressInfo.resolve("Filter ROI 1"));
        ...
        if (roi2Settings.isEnabled()) {
            ...
            copy1.addAll(roi1List);
            copy2.addAll(roi2List);
            IJ3DROIListData filtered = applyFilter(copy2, copy1, roi2Settings, imageHandler, variables, progressInfo.resolve("Filter ROI 2"));
            iterationStep.addOutputData("ROI 2", filtered, progressInfo);
        }
    }

    private IJ3DROIListData applyFilter(IJ3DROIListData roi1List, IJ3DROIListData roi2List, ROIFilterSettings settings, ImageHandler imageHandler, JIPipeExpressionVariablesMap variables, JIPipeProgressInfo progressInfo) {
