...
@SetJIPipeDocumentation(name = "Filter IJ3D ROI by overlap (old)", description = "Filters the 3D ROI lists by testing for mutual overlap. The ROI1 output contains all ROI1 input ROI that overlap with any of ROI2. " +
...
@AddJIPipeOutputSlot(value = Ij3dSuiteRoiListData.class, name = "ROI 1", create = true)
@AddJIPipeOutputSlot(value = Ij3dSuiteRoiListData.class, name = "ROI 2", create = true)
public class FilterRoi3dByOverlapOldAlgorithm extends JIPipeIteratingAlgorithm {
    ...
    public FilterRoi3dByOverlapOldAlgorithm(JIPipeNodeInfo info) {
        super(info);
    ...
    public FilterRoi3dByOverlapOldAlgorithm(FilterRoi3dByOverlapOldAlgorithm other) {
        super(other);
    ...
    private void updateSlots() {
        if (roi1Settings.isEnabled()) {
            if (!getOutputSlotMap().containsKey("ROI 1")) {
                ...
                slotConfiguration.addOutputSlot("ROI 1", "The first set of ROI", Ij3dSuiteRoiListData.class, false);
            }
        } else {
            if (getOutputSlotMap().containsKey("ROI 1")) {
                JIPipeDefaultMutableSlotConfiguration slotConfiguration = (JIPipeDefaultMutableSlotConfiguration) getSlotConfiguration();
        ...
        if (roi2Settings.isEnabled()) {
            if (!getOutputSlotMap().containsKey("ROI 2")) {
                JIPipeDefaultMutableSlotConfiguration slotConfiguration = (JIPipeDefaultMutableSlotConfiguration) getSlotConfiguration();
                slotConfiguration.addOutputSlot("ROI 2", "The second set of ROI", Ij3dSuiteRoiListData.class, false);
            }
        } else {
            if (getOutputSlotMap().containsKey("ROI 2")) {
                JIPipeDefaultMutableSlotConfiguration slotConfiguration = (JIPipeDefaultMutableSlotConfiguration) getSlotConfiguration();
    ...
    protected void runIteration(JIPipeSingleIterationStep iterationStep, JIPipeIterationContext iterationContext, JIPipeGraphNodeRunContext runContext, JIPipeProgressInfo progressInfo) {
        ...
        if (roi1Settings.isEnabled()) {
            ...
            Ij3dSuiteRoiListData filtered = applyFilter(copy1, copy2, roi1Settings, imageHandler, variables, progressInfo.resolve("Filter ROI 1"));
        ...
        if (roi2Settings.isEnabled()) {
            ...
            copy1.addAll(roi1List);
            copy2.addAll(roi2List);
            Ij3dSuiteRoiListData filtered = applyFilter(copy2, copy1, roi2Settings, imageHandler, variables, progressInfo.resolve("Filter ROI 2"));
            iterationStep.addOutputData("ROI 2", filtered, progressInfo);
        }
    }

    private Ij3dSuiteRoiListData applyFilter(Ij3dSuiteRoiListData roi1List, Ij3dSuiteRoiListData roi2List, ROIFilterSettings settings, ImageHandler imageHandler, JIPipeExpressionVariablesMap variables, JIPipeProgressInfo progressInfo) {
        ...
        for (int row = 0; row < measurements.getRowCount(); row++) {
            ...
            if (StringUtils.isNullOrEmpty(settings.overlapFilter.getExpression())) {
                if (settings.requireColocalization && settings.preciseColocalization) {
                    ...
                    Ij3dSuiteRoi roi1 = roi1List.get(roi1Index);
                    Ij3dSuiteRoi roi2 = roi2List.get((int) measurements.getValueAsDouble(row, "Roi2.Index"));
